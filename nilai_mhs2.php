...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		$sqlCek="SELECT * FROM tb_nilai_mhs1 WHERE NIM='$NIM'";
		...
		if(count($message)==0){			
			$sqlSave="INSERT INTO tb_nilai_mhs1 SET NIM='$NIM', NAMA='$txtNama', KODE_MATKUL='$cmbLevel', NILAI_TUGAS='$tgs', 
			NILAI_TTS ='$TTS', NILAI_TAS ='$TAS'";
			$qrySave=mysql_query($sqlSave, $connection) or die ("Gagal query".mysql_error());
			if($qrySave){
				echo "<meta http-equiv='refresh' content='0; url=?page=Data-Berita2&Act=Sucsses'>";
			}
		}	
