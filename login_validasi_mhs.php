...
if($_GET) {
	if($_POST) {
		...
		if(count($message)==0){	
			...
			$loginSql = "SELECT * FROM user_login_mhs WHERE namaU='".$txtUser."' AND passU='".md5($txtPassword)."' AND level='$cmbLevel'";
			...
			if($loginQry){
				if (mysql_num_rows($loginQry) >=1) {
					...
					if($cmbLevel=="Admin") {
						...
						$_SESSION['SES_LOGIN'] = $loginData['namaU']; 
					}
					
					// Jika yang login Dosen
					if($cmbLevel=="Dosen") {
						$_SESSION['SES_Dosen'] = "Dosen";
						$_SESSION['SES_LOGIN'] = $loginData['namaU']; 
					}
					
					//Jika login Mahasiswa
					if($cmbLevel=="MHS") {
						$_SESSION['SES_MHS'] = "MHS";
						$_SESSION['SES_LOGIN'] = $loginData['namaU']; 
					}
