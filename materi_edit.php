...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		$sqlCek="SELECT * FROM materi WHERE kode_K='$Kode_K' AND NOT(kode_K='$Kode_KLm')";
		...
		if(count($message)==0){
			$sqlSave="UPDATE materi SET kode_K='$Kode_K', Kode_F='$Kode_Fasil', Materi='$materi', Waktu='$waktu
			WHERE IdG='".$_POST['txtKode']."'";
	...
	$sqlShow = "SELECT * FROM materi WHERE IdG='$KodeEdit'";
	$qryShow = mysql_query($sqlShow, $connection)  or die ("Query ambil data user salah : ".mysql_error());
	$dataShow = mysql_fetch_array($qryShow);
	
	# MASUKKAN DATA KE VARIABEL
	$dataKode			= $dataShow['IdG'];
	$dataKode_K			= isset($dataShow['kode_K']) ?  $dataShow['kode_K'] : $_POST['Kode_K'];
	$dataKode_K			= $dataShow['Kode_K'];
	$dataKode_Fasil  	= isset($dataShow['Kode_F']) ?  $dataShow['Kode_F'] :  $_POST['Kode_Fasil'];
	$datamateri			= isset($dataShow['Materi']) ?  $dataShow['Materi'] : $_POST['materi'];
	$datawaktu		  	= isset($dataShow['Waktu']) ?  $dataShow['Waktu'] :  $_POST['waktu'];
	
} // Penutup GET
?>
 <form action="?page=Edit-Materi&Act=Save" method="post" name="frmadd">
...
<tr>
    ...
    <tr>
      ...
      <input name="Kode_K" type="hidden" value="<?php echo $dataKode_K; ?>" /></td>
    </tr>
    
    <tr>
	  <td width="15%"><strong>Kode Fasilitator : </strong></td>
	  <td width="1%"><b>:</b></td>
	  <td width="84%"><input name="Kode_Fsil"  value="<?php echo $dataKode_Fasil; ?>"  size="30" maxlength="30"/>
      <input name="Kode_Fasil" type="hidden" value="<?php echo $dataKode_Fasil; ?>" /></td>
    </tr>
    
    <tr>
      <td><strong>Materi : </strong></td>
	  <td><b>:</b></td>
	  <td><input name="materi" value="<?php echo $datamateri; ?>"  size="60" maxlength="60"/>
          <input name="materi" type="hidden" value="<?php echo $datamateri; ?>" /></td>
    </tr>
    
    <tr>
	  <td width="15%"><strong>Waktu : </strong></td>
	  <td width="1%"><b>:</b></td>
	  <td width="84%"><input type="time" name="waktu"  value="<?php echo $datawaktu; ?>"  size="30" maxlength="30"/>
      <input type="time" name="waktu" type="hidden" value="<?php echo $datawaktu; ?>" /></td>
    </tr>
